...
import { 
    ...
    FaUtensils, FaShoppingCart, FaCheck, FaClock, FaStar, FaThLarge
} from 'react-icons/fa';
...
const UnifiedDashboard = () => {
  ...
  const isFinance = activeModule === 'finance';
  const isProductivity = activeModule === 'productivity';
  ...
  }, [todos, habits, habitLogs, todayStr, isFinance]);

  const recentActivity = useMemo(() => {
    try {
        let activity = [];
        if (isFinance || !activeModule) {
            activity.push(...(meals || []).map(m => ({ ...m, actType: 'expense', icon: <FaUtensils className="text-warning"/>, label: m.item || 'Meal', date: m.date || todayStr })));
            activity.push(...(purchases || []).map(p => ({ ...p, actType: 'expense', icon: <FaShoppingCart className="text-danger"/>, label: p.item || 'Purchase', date: p.date || todayStr })));
            activity.push(...(incomes || []).map(i => ({ ...i, actType: 'income', icon: <FaArrowUp className="text-success"/>, label: i.source || 'Income', date: i.date || todayStr })));
        }
        if (isProductivity || !activeModule) {
            activity.push(...(todos || []).filter(t => t.completed).map(t => {
                let d = todayStr;
  ...
  }, [meals, purchases, incomes, todos, todayStr, isFinance, isProductivity, activeModule]);
  ...
  return (
    <div className="unified-dash pb-5 px-3 px-md-0">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        {/* Top Hero Section: Contextual Focus */}
        <div className="d-flex justify-content-between align-items-center mb-4">
            <h2 className="fw-bold mb-0">{isFinance ? 'Wealth Architect' : (isProductivity ? 'Peak Performance' : 'Unified Hub')}</h2>
            <Button variant="outline-secondary" size="sm" className="rounded-pill px-3 fw-bold border-dashed" onClick={() => navigate('/selection')}>
                <FaThLarge className="me-2" /> Global Hub
            </Button>
        </div>
        <Row className="g-4 mb-4">
          {(isFinance || !activeModule) && (
            <Col lg={isFinance ? 12 : 6}>
              <div className="hero-card finance shadow-sm p-4 h-100" style={{ background: 'var(--primary-gradient)', borderRadius: '2rem' }}>
                <div className="d-flex justify-content-between align-items-start mb-3">
                  <div>
                    <small className="text-white opacity-75 text-uppercase fw-bold letter-spacing-1">Current Capital</small>
                    <h1 className="display-5 fw-bold text-white mb-0 financial-stat">{formatCurrency(globalStats?.totalBalance)}</h1>
                  </div>
                ...
                <div className="d-flex gap-3 align-items-center mt-4">
                  ...
                  <Button variant="light" size="sm" className="rounded-pill px-3 fw-bold ms-auto" onClick={() => navigate('/wallets')}>Capital Audit</Button>
                </div>
              </div>
            </Col>
          )}
          {(isProductivity || !activeModule) && (
            <Col lg={isProductivity ? 12 : 6}>
              <div className="hero-card productivity shadow-sm p-4 h-100" style={{ background: 'linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)', borderRadius: '2rem' }}>
                <div className="d-flex justify-content-between align-items-start mb-3">
                  <div>
                    <small className="text-white opacity-75 text-uppercase fw-bold letter-spacing-1">Daily Discipline</small>
                ...
                <div className="mt-4">
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    <div className="small text-white fw-medium">Peak state achieved: {avatarState?.streak || 0} days</div>
                    <Button variant="light" size="sm" className="rounded-pill px-3 fw-bold" onClick={() => navigate('/planner')}>Focus Mode</Button>
                  </div>
                </div>
              </div>
            </Col>
        ...
        {/* Quick Action Launchpad: Context Aware */}
        <div className="launchpad mb-4 d-flex gap-3 pb-2 overflow-auto" style={{ scrollbarWidth: 'none' }}>
          {isFinance && (
            <Button variant="primary" className="launch-btn flex-shrink-0" onClick={() => openTransactionModal()}>
              <FaPlus className="me-2" /> Log Asset Shift
            </Button>
          )}
          {isProductivity && (
            <Button variant="primary" className="launch-btn flex-shrink-0 bg-purple border-0" onClick={() => navigate('/planner')}>
              <FaTasks className="me-2" /> Commit Task
            </Button>
          )}
          {isFinance && (
            <Button variant="outline-primary" className="launch-btn flex-shrink-0" onClick={() => navigate('/goals')}>
              <FaPiggyBank className="me-2" /> Secure Goal
            </Button>
          )}
          {isProductivity && (
            <Button variant="outline-primary" className="launch-btn flex-shrink-0 text-purple border-purple" style={{ borderColor: '#8b5cf6' }} onClick={() => navigate('/productivity/habits')}>
              <FaFire className="me-2" /> Track Routine
            </Button>
          )}
          <Button variant="outline-secondary" className="launch-btn flex-shrink-0" onClick={() => navigate('/settings')}>
             Configuration
          </Button>
        </div>
        ...
        <Row className="g-4">
          <Col lg={8}>
            ...
            {/* Selection Hub Cards - Hide in modules for cleaner UI */}
            {!activeModule && (
              <Row className="g-4 mb-4">
                <Col md={6}>
              ...
              <Card.Body className="p-4">
                  ...
                  {(isFinance || !activeModule) && (
                    <Col md={isFinance ? 12 : 6}>
                      <div className="snapshot-item p-3 rounded-4 bg-light bg-opacity-10 border">
                        ...
                        <p className="x-small text-muted mt-2 mb-0">Monthly budget threshold</p>
                      </div>
                    </Col>
                  )}
                  {(isProductivity || !activeModule) && (
                    <Col md={isProductivity ? 12 : 6}>
                      <div className="snapshot-item p-3 rounded-4 bg-light bg-opacity-10 border">
                        <div className="d-flex justify-content-between mb-2">
                          <small className="fw-bold text-muted">Hydration Metric</small>
                          <small className="fw-bold text-info">{waterStats?.current || 0} / {waterStats?.goal || 8}</small>
                        </div>
                        <ProgressBar now={((waterStats?.current || 0) / (waterStats?.goal || 8)) * 100} variant="info" style={{ height: '8px' }} className="rounded-pill" />
                        <p className="x-small text-muted mt-2 mb-0">Daily hydration target</p>
                      </div>
                    </Col>
                  )}
                </Row>
                ...
                <div className="ai-insight mt-4 p-3 rounded-4 bg-primary bg-opacity-10 border border-primary border-opacity-25 d-flex gap-3 align-items-center">
                  ...
                  <div className="flex-grow-1">
                    <h6 className="fw-bold mb-1 small">Intelligent Protocol</h6>
                    <p className="x-small mb-0 text-muted">
                      {isFinance ? (burnRate > 70 ? "Protocol: Restrict non-essential capital outflow." : "Protocol: Capital threshold healthy. Direct surplus to growth targets.") : 
                       "Protocol: Peak focus mode recommended. Block financial distractions for the next 2 hours."}
                    </p>
                  </div>
                  <Button variant="link" size="sm" className="p-0 text-primary fw-bold text-decoration-none">Insights</Button>
                </div>
              </Card.Body>
            </Card>
          </Col>
          ...
          <Col lg={4}>
            ...
            {/* Daily Priorities - Only in Productivity */}
            {isProductivity && topPriorities.length > 0 && (
                <Card className="dash-card border-0 shadow-sm mb-4" style={{ borderRadius: '1.5rem', background: 'var(--card-bg)' }}>
                    <Card.Body className="p-4">
                        <h6 className="fw-bold mb-3 d-flex align-items-center gap-2">
                            <FaStar className="text-warning" /> High-Value Targets
                        </h6>
                            ...
                            {topPriorities.map((item, idx) => (
                                <ListGroup.Item key={idx} className="bg-transparent border-0 px-0 py-2 d-flex align-items-center gap-3">
                                    ...
                                    <div className="overflow-hidden">
                                        ...
                                        <div className="x-small text-muted">{item.type === 'todo' ? 'Priority Task' : 'Daily Routine'}</div>
                                    </div>
                                </ListGroup.Item>
                            ))}
                        </ListGroup>
            ...
            {/* Savings Goal Progress - Only in Finance */}
            {isFinance && primaryGoal && (
                <Card className="dash-card border-0 shadow-sm p-4 mb-4" style={{ borderRadius: '1.5rem', background: 'var(--card-bg)' }}>
                    <h6 className="fw-bold mb-3 d-flex align-items-center gap-2">
                        <FaPiggyBank className="text-info" /> Growth Milestone
                    </h6>
                    <div className="d-flex justify-content-between mb-2">
                    ...
                    <Button variant="outline-info" size="sm" className="w-100 rounded-pill mt-3 fw-bold" onClick={() => navigate('/goals')}>Deploy Capital</Button>
                </Card>
            )}
            ...
            <Card className="dash-card border-0 shadow-sm mb-4" style={{ borderRadius: '1.5rem', background: 'var(--card-bg)' }}>
                <Card.Body className="p-4">
                    <h6 className="fw-bold mb-3 d-flex align-items-center gap-2">
                        <FaClock className="text-primary" /> Activity Audit
                    </h6>
                    <ListGroup variant="flush">
                        {recentActivity.map((act, idx) => (
                            <ListGroup.Item key={idx} className="bg-transparent border-0 px-0 py-2 d-flex align-items-center gap-3">
                                ...
                                <div className="overflow-hidden">
                                    ...
                                    <div className="x-small text-muted amount-text">{act.actType === 'expense' ? `Spent ${formatCurrency(act.amount)}` : act.actType === 'income' ? `Earned ${formatCurrency(act.amount)}` : 'Completed'}</div>
                                </div>
                            </ListGroup.Item>
                        ))}
                    </ListGroup>
